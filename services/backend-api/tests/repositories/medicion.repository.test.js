...
describe('medicion.repository', () => {
  ...
  it('guarda una medición recibida del gateway (incluyendo temperatura_corporal y spo2) y la recupera por id', async () => {
    const payload = medicionMock(seudonimo.id, dispositivo.id);
    const creada = await medicionRepository.crear(payload);

    expect(creada).toMatchObject({
      id_seudonimo: seudonimo.id,
    ...
    expect(recuperada).toMatchObject({ id: creada.id, id_seudonimo: seudonimo.id });
  });
  ...
  it('lista mediciones de un seudónimo filtrando por rango de fechas', async () => {
    await medicionRepository.crear(
      medicionMock(seudonimo.id, dispositivo.id, { fechaHora: new Date('2026-01-01T10:00:00Z') }),
    );
    await medicionRepository.crear(
      medicionMock(seudonimo.id, dispositivo.id, { fechaHora: new Date('2026-06-01T10:00:00Z') }),
    );

    const enRango = await medicionRepository.listarPorSeudonimo(seudonimo.id, {
  ...
  it('vía el service, guarda una medición válida end-to-end asociada al seudónimo, no al operario', async () => {
    ...
    expect(guardada.id_seudonimo).toBe(seudonimo.id);
    expect(guardada.id_trabajador).toBeUndefined();

    const listado = await medicionesService.listarMedicionesDeTrabajador(trabajador.id);
    expect(listado).toHaveLength(1);
    expect(listado[0].id_seudonimo).toBe(seudonimo.id);
  });
});
