...
- **Válido** → `201` con la medición persistida. `fecha_hora` = timestamp de **recepción** asignado por el backend (RF-03); `fecha_hora_captura` conserva el timestamp del dispositivo como dato adicional.
...
| 4 | Duplicado: mismo `id_dispositivo` + misma `fecha_hora_captura` | `DUPLICADO` | 409 | Sí | H0008 "no se aceptan registros duplicados" (criterio documentado en la migración `20260719000001`) |
...
- **Criterio de duplicado**: el DER v1.1 no lo define y "wearable + timestamp de recepción" es vacuo (lo asigna el backend, nunca colisiona). Se usa `id_dispositivo + fecha_hora_captura`, el duplicado real que producen la cola local y los reintentos del gateway. Índice único parcial → detección O(log n), sin escaneos (RNF-01), y defensa ante carreras (`ON CONFLICT DO NOTHING`).
