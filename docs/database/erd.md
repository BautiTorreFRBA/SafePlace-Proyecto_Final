...
4. **`medicion.fecha_hora_captura` (`TIMESTAMPTZ`, nullable) + índice único parcial `(id_dispositivo, fecha_hora_captura)`.** Soporte del Servicio de Validación de Datos (RF-04/H0008): `fecha_hora` es la marca oficial de recepción asignada por el backend (RF-03) y `fecha_hora_captura` conserva la marca del dispositivo como dato adicional. El índice único materializa el criterio de duplicado (mismo wearable + misma captura), que el DER v1.1 no define. Ver `20260719000001_add-medicion-fecha-captura.js`.
